                            ...
                            <div class="row">
                                <div class="col-md-8 col-sm-8">
                                    ...
                                    <div class="clearfix"></div>
                                        ...
                                        <br>
                                        <br>
                                    @if($job->expiry_date < date('Y-m-d H:i:s'))
                                     <p><h6 style="color: red"> {{__('EXPIRED JOB, CLICK RELAUNCH BUTTON TO PUBLISH AGAIN')}} </h6></p>
                                    @endif
                                    @if(($job->is_featured) && $job->expiry_date >= date('Y-m-d H:i:s'))
                                        <p><h6 style="color: lightsalmon"> {{__('THIS JOB IS IN PREMIUM POSITION')}} </h6></p>
                                    @endif
                                </div>
                                <div class="col-md-4 col-sm-4">                
                                    ...
                                    @endif
                                    @if($job->expiry_date < date('Y-m-d H:i:s'))
                                    <div class="listbtn"><a href="{{route('relaunch.front.job', [$job->id])}}">{{__('RELAUNCH')}}</a></div>
                                    @endif
                                    <div class="listbtn"><a href="javascript:;" onclick="deleteJob({{$job->id}});">{{__('Delete')}}</a></div>
                                    @if(!$job->is_featured)
                                    <div class="listbtn"><a href="{{route('company.buy.premium', [$job->id])}}">{{__('BUY PREMIUM POSITION')}}</a></div>
                                    @endif
                                </div>
                            </div>
