...
<div class="page-content-wrapper"> 
    ...
    <div class="page-content"> 
        ...
        <!-- END PAGE BAR --> 
        <!-- BEGIN PAGE TITLE-->
        <h3 class="page-title">Manage NEWS <small>{{__('NEWS')}}</small> </h3>
        <!-- END PAGE TITLE--> 
        <!-- END PAGE HEADER-->
        <div class="row">
            <div class="col-md-12"> 
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        <div class="caption"> <i class="icon-settings font-dark"></i> <span class="caption-subject font-dark sbold uppercase">NEWS</span> </div>
                        <div class="actions"> <a href="{{ route('create.news') }}" class="btn btn-xs btn-succes"><i class="glyphicon glyphicon-plus"></i> Add New NEWS Page</a> </div>
                    </div>
                    <div class="portlet-body">
                        <div class="table-container">
                            <form method="post" role="form" id="news-search-form">
                                <table class="table table-striped table-bordered table-hover"  id="news_datatable_ajax">
                                    <thead>
...
<script>
    $(function () {
        var oTable = $('#news_datatable_ajax').DataTable({
            procesing: true,
            serverSide: true,
            stateSave: true,
            searching: false,
        ...
        if (confirm('Are you sure! you want to delete? All content pages will be deleted too.')) {
            $.post("{{ route('delete.cms') }}", {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
                    .done(function (response) {
                        if (response == 'ok')
                        {
                            var table = $('#news_datatable_ajax').DataTable();
