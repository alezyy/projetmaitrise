...
class OrderController extends Controller
{
    ...
    public function orderPackage(Request $request, $package_id)
    {
        ...
        $order_amount = number_format($package->package_price + $package->package_price * env('TPS')
            + $package->package_price * env('TVQ'), 2,'.', '');
     ...
     * @return IlluminateHttpResponse
     */
    public function orderPremiumPackage(Request $request, $package_id)
    {
         $param = $request->route()->parameters();
         session(['jobId' => $param['jobId']]);
        ...
        $transaction->setAmount($amount)
            ...
            ->setDescription($description);
        $redirect_urls = new RedirectUrls();
        $redirect_urls->setReturnUrl(URL::route('payment.premium.status', $package_id)) /** Specify return URL * */
        ->setCancelUrl(URL::route('payment.premium.status', $package_id));
        $payment = new Payment();
        $payment->setIntent('Sale')
            ->setPayer($payer)
            ->setRedirectUrls($redirect_urls)
    ...
    public function getPaymentPremiumStatus(Request $request, $package_id)
    {
        $package = Package::findOrFail($package_id);
        $gateway = 'Paypal';
