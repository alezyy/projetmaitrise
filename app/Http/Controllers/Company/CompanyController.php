...
class CompanyController extends Controller
{
    ...
    {
        $data['jobId'] =$id;
        return view('company_premium')->with($data);
    }
    ...
    public function companyTransactions()
    {
        ...
        return view('company.company_transactions')->with($data);
    }

    public function companyTransacPdf($id)
    {
        $data['tx_id'] = $id;
        $company = Company::findOrFail(Auth::guard('company')->user()->id);
        $data['country'] = Country::findOrFail($company->country_id)->country;
        $data['state'] = State::findOrFail($company->state_id)->state;
        $data['city'] = City::findOrFail($company->city_id)->city;
