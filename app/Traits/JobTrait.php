...
trait JobTrait
{
    ...
    private function assignJobValues($job, $request)
    {
        ...
        $job->language_id = $request->input('language_id');
        return $job;
    }
    ...
    public function createFrontJob()
    {
        ...
        if ((bool)$company->verified === false) {
            flash(__('Please verify your account, We sent you an email! '))->error();
    ...
    public function premiumFrontJob($id)
    {
        $job = Job::findOrFail($id);
        $job->is_featured = 1;
        $job->end_featured_date = date('Y-m-d H:i:s', strtotime('+5 days'));
        //$job = $this->assignJobValues($job, $request);
        /*         * ******************************* */
        $job->slug = str_slug($job->title, '-') . '-' . $job->id;
        /*         * ******************************* */
    ...
    public function premiumPaypalJob($id)
    {
        $job = Job::findOrFail($id);
        $job->is_featured = 1;
        $job->end_featured_date = date('Y-m-d H:i:s', strtotime('+5 days'));
        ...
        //$this->storeJobSkills($request, $job->id);
        /*         * ************************************ */
        // return $this->updateFullTextSearch($job);
        /*         * ************************************ */
        //flash('Job has been updated!')->success();
        //return \Redirect::route('edit.front.job', array($job->id));
    }

    public function relaunchFrontJob($id)
    {
        ...
        $job = Job::findOrFail($id);
        $job->expiry_date = date('Y-m-d H:i:s', strtotime("+30 days"));
        //$job = $this->assignJobValues($job, $request);
        /*         * ******************************* */
        $job->slug = str_slug($job->title, '-') . '-' . $job->id;
        /*         * ******************************* */
        ...
        flash('Job has been published again!')->success();
        return \Redirect::route('edit.front.job', array($job->id));
    }
